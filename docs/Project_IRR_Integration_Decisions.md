...
1. **Fuel escalation starts year 1, not year 4** in Excel — would explain part of the year 1-3 PPA-period gap. Pure opex effect.
2. **`gas_mwh` varies year-on-year** in Excel (gas degradation? Variable maintenance schedule? Load growth?) — would explain non-monotonic year-on-year ratios. If true, the same mechanism likely touches gas *revenue* too (PPA + merchant), making this potentially a revenue-side bonus.

**Decision deferred**: Park r34 fix pending root-cause diagnostic (next session). Direction-of-impact analysis says fixing r34 alone would:

- Reduce Gas-only IRR (currently 14.13% vs Excel 10.77%, over by +3.36 pp) — **helpful direction** for the Gas-only structural validator.
- Reduce Combined IRR (currently 8.78% vs target 9.2%, under by -0.42 pp) — **widens** the gap.

So r34 is NOT a Combined-gap-closer in isolation. But if root cause is gas degradation (not fuel-esc-start), the symmetric revenue-side correction could be a Combined gain. Worth one more diagnostic before parking permanently.
