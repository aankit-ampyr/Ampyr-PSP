...
| 2026-05-26 | **A51 added: PPA tariff / tenor / escalation surfaced in Step 2b UI** (3 new `st.number_input` widgets at top of the PPA expander). Closes a pre-existing legacy gap — the engine adapter reads `ppa_tariff_gbp_mwh`, `ppa_tenor_years`, `ppa_escalation_pct` directly (D13 defaults £170 / 10 / 0%) but no UI was wired to write them. The existing PPA widgets (`ppa_selection`, `ppa_flex_pct`, `ppa_indexation`) were saved-to-state but engine-ignored (vestiges of a pre-engine-wiring "case-table lookup" design that never landed). A51 keeps the legacy widgets visible but clearly labels them "— legacy" with a caption explaining they don't drive the engine. Save payload extended to include the 3 primary fields. Test fixture `_step2b_saved_fin_with_current_defaults` extended for completeness; new e2e test `test_ppa_tariff_override_lifts_pirr` proves the override flows through the adapter (doubling tariff £170 → £340 lifts Combined PIRR ≥ +1 pp). **D13 audit invariant preserved**: 4 audit tests still pass at the default values. Tests: **74 pass, 0 xfail** (was 73 — +1 new). Note: PPA tariff link to gas (A31) means doubling solar PPA also doubles gas PPA → both revenue streams shift; user changing tariff sees the full Excel-faithful behaviour. | Surfaced during the A50 follow-up conversation when the user asked "where is the tariff input configured" — the gap was diagnosed and the missing widgets added. |
...
| 2026-05-16 | **A40 added: time-varying CPI curve wired into `_esc_factor` (Excel-faithful per Guardrail 5; null-result for IRR).** A39 flattened the engine CPI rate to 2.0% (Excel steady-state from 2030 onwards), missing the early-year detail (2025=3.1%, 2026=2.5%, 2027=2.2%, 2028=2.2%, 2029=2.1%). A40 implements the full year-by-year curve. **Code changes**: (1) `_DEFAULT_CPI_CURVE_BY_CALENDAR_YEAR` module constant (calendar-year-indexed dict; years not present fall back to `rates["CPI"]` = 2.0%). (2) `_build_cpi_factor_lookup(curve, steady_state, cod_year, n_ops_years)` precomputes per-ops-year cumulative factor list. (3) `_esc_factor` accepts optional `cpi_factor_lookup` parameter and uses it for "CPI" case when provided (falls back to flat rate otherwise; backward-compatible). (4) `_calc_revenue` and `_calc_opex` thread the lookup through all 11 "CPI"-using call sites (REGO, embedded, CM T-1/T-4, BESS floor, solar fixed, PV O&M, corrective maint, balancing CfD, BESS opex, fixed lease). (5) `_run_pirr_core` builds the lookup once. (6) New `PirrInputs.cpi_curve_by_calendar_year` field defaulting to the Burton-Leonard Excel curve. **Result: audit unchanged at 2-decimal precision** (d13 Combined 8.88% / S+B 9.05% / Gas 13.07%; sub-bp shift at 4-decimal precision). The curve adds ~0.3% to lifetime CPI factor over flat 2.0% (years 1-3 are 2.1-2.2% vs steady-state 2.0%, but steady state covers 31 of 35 ops years). Tests: 42 pass + 4 xfail; no re-baselining needed (within tolerance ±0.1 pp). **Why ship despite null result**: Guardrail 5 — replicate Excel mechanism, never use a fudge factor. Curve infrastructure also unlocks future configs/SME questions (e.g. if Anchal confirms a different curve for Insurance). | A39 follow-up; Excel-faithful mechanism, audit gap unchanged. |
...
| 2026-05-16 | **Session paused for SME handover.** Memo drafted at [docs/SME_Memo_Anchal_2026-05-16.md](SME_Memo_Anchal_2026-05-16.md) with Q1 (Insurance NIL contradiction) and Q2 (audit tolerance ±0.1 pp vs ±0.3 pp). Memo SENT to Anchal via Teams. Engine state: d13 Combined 8.88% / S+B 9.05% / Gas 13.07%; 42 pass + 4 xfail. Queued for next session: time-varying CPI curve (~+2-3 bps, ~1-2 hr) if Anchal hasn't replied; otherwise apply her answer. Working tree clean post-`5a9a52c`. | End-of-session handover. No engine changes. |
...
### A51. PPA tariff / tenor / escalation surfaced in Step 2b UI (NEW 2026-05-26)

Pre-A51, the PIRR adapter `pirr_inputs_from_wizard_state` read three PPA fields directly from `wizard['financial']`:

```python
ppa_tariff_gbp_mwh = f("ppa_tariff_gbp_mwh", 170.0)   # Excel: Overall Inputs!E13
ppa_tenor_years   = i("ppa_tenor_years", 10)          # Excel: Overall Inputs!E11
ppa_escalation_rate = f("ppa_escalation_pct", 0.0) / 100.0   # Excel: Overall Inputs!E14
```

The defaults in `DEFAULT_WIZARD_STATE['financial']` matched D13 (£170 / 10 / 0%) and the audit tests passed. But **no UI widget wrote to these keys** — they were locked at the wizard-state defaults regardless of user input.

The Step 2b PPA expander had three widgets (`ppa_selection`, `ppa_flex_pct`, `ppa_indexation`) — but none drove the engine. They were vestiges of a pre-engine-wiring design where `ppa_selection = 1, 2, ...` was supposed to map to specific tariff levels via a case-table lookup that never landed. The save button wrote these to wizard state; the adapter ignored them.

This gap was diagnosed when the user asked "where is the tariff input configured" during the A50 follow-up. A51 closes it.

**Changes:**

1. **`pages/Step2b_FinancialSetup.py` PPA expander** — restructured into two layers:
   - **Primary inputs (3 columns)** at the top, each labelled with Excel cell ref + D13 default:
     - `PPA Tariff (£/MWh)` → writes `ppa_tariff_gbp_mwh` (default 170.00)
     - `PPA Tenor (years)` → writes `ppa_tenor_years` (default 10)
     - `PPA Escalation (%/yr)` → writes `ppa_escalation_pct` (default 0.00)
   - **Legacy widgets (2 columns)** below, with explicit "— legacy" suffix on labels and a caption flagging that the engine ignores them. Kept for backwards compatibility with stored sessions; safe to remove if a future cleanup pass deems them dead.

2. **Save payload** in the "Save Financial Inputs" button extended to include the 3 primary fields:
   ```python
...
3. **Test fixture** `_step2b_saved_fin_with_current_defaults` in `tests/test_wizard_state_path.py` extended to include the 3 primary fields at their D13 defaults. Locks alignment.

4. **New e2e test** `test_ppa_tariff_override_lifts_pirr`: sets `fin['ppa_tariff_gbp_mwh'] = 340.0` (double the default), confirms the adapter passes it through to `PirrInputs.ppa_tariff_gbp_mwh`, and asserts Combined PIRR rises by at least +1 pp. Proves the new UI widget actually drives the engine.

**Behavioural notes:**

- **PPA tariff link to gas (A31)**: Excel `Inputs-Gas!I19 = 'Overall Inputs'!E13`. The engine replicates this — changing `ppa_tariff_gbp_mwh` also changes the gas PPA tariff during gas's 10-year PPA tenor. Users editing the new widget see the full Excel-faithful sensitivity (engine response ~1.5 pp Combined per £10/MWh change for D13, confirmed by A31).
- **D13 audit invariant preserved**: when both UI defaults (£170 / 10 / 0%) match the test fixture and `DEFAULT_WIZARD_STATE`, the engine produces the same numbers. All 4 audit tests, 3 Step 2b-saved tests, and 3 fresh-session tests still pass green.
- **PPA indexation** widget is still kept as legacy. The engine uses `ppa_escalation_pct` directly (a scalar) rather than the indexation dropdown's enum. To wire indexation, a future change would map "CPI" / "RPI" / "Fixed" / "Blend" to the engine's escalation logic; out of scope for A51.

**Tests**: 73 → **74 pass, 0 xfail** (+1 new e2e regression).

**Out of scope for A51** (still on the table for follow-up if needed): wire the legacy `ppa_indexation` dropdown to drive escalation; or remove the 3 legacy widgets entirely.
...
A48 (same day) shipped a placeholder upload panel on Step 1 ("💰 Market Price Curve") that visualised the engine default curve, accepted a CSV upload, and stashed it in `wizard['setup']['merchant_price_curve']` — but the engine adapter never read it. The panel caption explicitly admitted *"engine still reads its locked Burton Leonard default; uploading a custom curve stores it for a future release."* A49 closes that loop.

**Scope deliberately limited.** The curve-scope question parked in A48 had two possible answers:
- **Minimal** (this PR): one toggle — *use engine default or upload your own nominal curve*. No real-terms inputs, no CPI curve editing.
- **Maximal**: support real-terms uploads, separate CPI curve uploader on Step 2b, real→nominal conversion in the adapter, plus a 3-line preview chart showing the transformation.

User chose the minimal path with the maximal plan archived in [docs/future_improvements/v2_price_and_inflation_curves.md](../docs/future_improvements/v2_price_and_inflation_curves.md) for v2. Rationale: uploaded forecaster curves typically arrive in real terms, so the maximal version is genuinely more useful — but it requires a sibling inflation-curve editor on Step 2b, a new pure helper in the engine, and a substantial UI rebuild. A49 ships the minimum useful surface (Aurora / Baringa users who already convert to nominal in Excel before upload) and leaves the conversion piece for a focused v2 session.

**Engine change (the only non-trivial bit).** [src/project_irr.py:1816+](../src/project_irr.py#L1816) — inside `pirr_inputs_from_wizard_state`, immediately before the `PirrInputs(...)` constructor:
...
if price_source == 'upload' and uploaded_curve:
    effective_merchant_prices = dict(uploaded_curve)   # nominal £/MWh, monthly
else:
    effective_merchant_prices = dict(_DEFAULT_MERCHANT_PRICES_MONTHLY)
```

Then `merchant_prices_monthly=effective_merchant_prices` added to the constructor kwargs near the PPA section. On the `'default'` path, `effective_merchant_prices` is a fresh copy of the same locked Burton-Leonard dict the dataclass factory would have built — so the D13 audit numbers are pinned by construction.

**G5 compliance.** No transformation, no calibration constant, no escalation applied. Values passed through verbatim. The whole point of treating the upload as *nominal* is that it matches what the engine has always consumed (`_DEFAULT_MERCHANT_PRICES_MONTHLY` is itself the locked nominal-terms snapshot from Excel `Solar&BESS Operation!row 66` per A30). The Excel chain that produces nominal prices from Aurora/Baringa real-terms forecasts (`Curves and D&T!r30` indexation) is **not** modelled — users must do that conversion offline before upload, or wait for v2.

**Step 1 panel changes** ([pages/Step1_Setup.py:583-770](../pages/Step1_Setup.py#L583)):

| Change | Detail |
|---|---|
| Subheader | "💰 Market Price Curve" → "💰 Nominal Merchant Curve" |
| Caption | Explicitly notes "**nominal terms — inflation already applied**" + references Excel `Solar&BESS Operation!row 66` as the locked default source |
| Radio labels | `default`: "Use engine default (Burton-Leonard `Solar&BESS Operation!r66`)". `upload`: "Upload custom nominal curve (CSV)" |
| Help text | "price_gbp_mwh = nominal £/MWh (must include inflation; engine does not escalate this curve)" + computed required-coverage range surfaced inline |
| Coverage validator | New `_compute_post_ppa_range(fin_dict)` + `_validate_curve_coverage(curve_dict, fin_dict)` helpers. Required coverage = every `(year, month)` from `(cod.year + ppa_tenor_years, cod.month)` through `(cod.year + project_life_years - 1, 12)`. Rejects incomplete uploads with `st.error` and **does not write to state** (stored curve unchanged) |
| Final caption | Removed "Placeholder — uploaded curves are stored but not yet read" notice. Replaced with v2 forward-pointer to [docs/future_improvements/v2_price_and_inflation_curves.md](../docs/future_improvements/v2_price_and_inflation_curves.md) |

**Why coverage validation matters.** `_merchant_price` (engine) silently falls back per-month to its locked default when a key is missing. A partial upload would mix the user's data with Burton-Leonard values without any warning — exactly the "I uploaded a curve but the numbers don't match Excel" surprise we want to avoid. The validator forces the user to either upload a full post-PPA window or stick with `default`. The required range falls back to D13 engine defaults (COD = 2027-07, ppa_tenor = 10, project_life = 35 → required range 2037-07 through 2061-12) when the user hasn't visited Step 2b yet.

**Tests** ([tests/test_wizard_state_path.py](../tests/test_wizard_state_path.py)):

| Test | Purpose |
|---|---|
| `test_default_curve_preserves_d13_audit` | Explicit `merchant_price_curve_source='default'` + `merchant_price_curve=None` reproduces D13 Combined 8.84% / S+B 9.00%. Locks the False side of the new adapter branch. |
| `test_uploaded_nominal_curve_feeds_engine` | Doubled-curve upload (every value × 2) shifts Combined PIRR ≥ +0.5 pp upward. Proves wiring works — would fail if the adapter still hard-coded the default. |

Both pass. Existing 4 wizard-state-path tests (`test_step2b_saved_*`, `test_default_wizard_state_produces_audit_*`, `test_wizard_state_d13_*`) cover the `'default'` branch implicitly and remain green. **52 tests pass, 0 xfail** (was 50 — +2 new).

**Files changed**: 5 — `src/project_irr.py` (adapter branch), `pages/Step1_Setup.py` (panel relabel + validator), `tests/test_wizard_state_path.py` (+2 tests), this file (A49 entry), `docs/Project_IRR_Status.md` (header update).

**Future-API hook**: not reserved. Per the v1 plan, sources stay `'default' | 'upload'`. When forecast API integration arrives, the enum extends additively — `'api'` value can be added without breaking stored sessions. No dead UI today.
...
**Why this was the right call for v1**:

1. **v1 was always scoped as ungeared Project IRR** per Spec D1/D2. Equity IRR (which DSCR sculpting feeds into) was always v2 territory. The audit gap is the projection of that scope decision onto the Combined PIRR metric.
2. **Every individual mechanism IS Excel-faithful** per Guardrail 5 — A21 (SHL + RB + CIR), A22 (UKETS thermal + SHL gas exclusion), A29-A40 (merchant prices, gas PPA link, balancing split, land lease, PV O&M escalation, gas major maint schedule, multi-account dep, NOL pool, CPI rate, time-varying CPI curve), A44 (Insurance schedule). No fudge factors, no calibration constants.
3. **The gap is uniform**: -0.36 / -0.48 / -0.23 / -0.23 pp = range 0.25 pp. Direction always same (engine slightly conservative). Config ranking preserved across the matrix. Sensitivity (PPA, capacity sweep) reproduces correctly.
4. **Production handoff unblocked**: Doublu builds on this engine for Phase 1B/2. They inherit the same offset, not a different engine.

**Trade-off accepted for v1**:

| Engine GETS RIGHT | Engine GETS WRONG |
| --- | --- |
| Config ranking | Headline IRR vs Excel (0.3-0.5 pp pessimism) |
| Sensitivity analysis | No Equity IRR |
...
| | Pre-A45 | Post-A45 |
| --- | --- | --- |
| Tolerance | ±0.1 pp (`TOLERANCE_PP = 0.001`) | **±0.5 pp** (`TOLERANCE_PP = 0.005`) |
| `test_d13_audit_combined` | xfail | **pass** |
...
- `tests/test_project_irr_excel_parity.py` — `TOLERANCE_PP` 0.001 → 0.005; removed 4 `@pytest.mark.xfail` markers; docstring updated to reference A45 + v1 scope; standalone runner notes the v1 tolerance.
- `docs/Financial_Assumptions_Spec.md` — §9 audit criteria updated; §10 "Deferred to v2" extended with DSCR sculpting + Equity IRR + gearing-as-sweep.
- `docs/Project_IRR_Status.md` — v1 closure headline + uplift table extended with A45 row.
- `docs/SME_FollowUp_Anchal_2026-05-16_post-A44.md` — appended a "Decision: Path 1 chosen" note.
- `c:\repos\CLAUDE.md` (top-level project tracker) — PSP active work section updated to mark v1 audit CLOSED.

**Test count**: 49 passed (was 45 + 4 xfail; the 4 xfails converted to passes by tolerance relaxation, not by closing the underlying gap — be honest about this in any reporting).
...
**Trigger**: The first browser smoke test using the new §A playbook script (Claude-for-Chrome extension, 2026-05-16) failed at A.5.5. D13 row in Step 3a's ranked table reported **Combined 21.59% / CAPEX £64.7m**, not the expected 8.88% / £101.6m. Browser-agent's report flagged that £64.7m matched smoke test §13's signature (the pre-A28 `capex_bess` unit-mismatch bug).

**Why A28's fix wasn't enough**. A28 (2026-05-13) closed a Step-7-defaults-drift bug by:

1. Updating Step 7's UI `value=` defaults (so saved state would be correct);
2. Switching the engine adapter to read `corp_tax_rate_high` instead of `_low`;
3. Adding 3 lock-in tests in `tests/test_wizard_state_path.py` that simulate "user clicked Save Financial Inputs" and assert D13 numbers reproduce.

What A28 did NOT touch: `DEFAULT_WIZARD_STATE['financial']` in `src/wizard_state.py`. The lock-in test constructed the `fin` dict inline (`_step7_saved_fin_with_current_defaults`), so the test fixture itself was the canonical "A28-aligned values" snapshot — the actual wizard state initialization was never exercised by the test.

**Real-world consequence**. Any user who opens Step 3a without first visiting Step 7 + clicking "Save Financial Inputs" got the pre-A28 default values from `DEFAULT_WIZARD_STATE['financial']`. That's exactly what the browser-agent did (debug line confirmed `fin.enabled=False`, meaning Step 7-Save was never invoked). The 25 misaligned values + 5 missing keys produced the wrong engine inputs.

**Programmatic reproduction** (pre-A43):

```python
from copy import deepcopy
from src.wizard_state import DEFAULT_WIZARD_STATE
# ... build setup, load solar profile ...
fin = deepcopy(DEFAULT_WIZARD_STATE['financial']); fin['enabled'] = True
# Run dispatch + pirr_inputs_from_wizard_state + run_pirr
# → Combined 19.67%, S+B 24.01%, Gas 12.43%, CAPEX £64.7m
```

(IRR numbers differ slightly from the browser-agent's 21.59% / 28.21% — likely a small variation in dispatch parameters between the agent's path and this reproduction. CAPEX matches exactly: £64.7m, the structural smoking gun.)

**Fix — 25 misaligned values + 5 missing keys in `src/wizard_state.py`**:

Misalignments (each item: `key: pre-A43_value → post-A43_value`):

| Section | Changes |
| --- | --- |
| Solar | `generation_selection: 'P90' → 'P50'` |
| BESS | `bess_operating_life: 15 → 10` (Excel `Solar&BESS Inputs!F112`) |
| REGOs | `rego_price: 5.0 → 2.5`; `rego_indexation: 'CPI' → 'NIL'`; `rego_tenor_years: 15 → 35` |
| Capacity Market | `cm_t1_value: 20.0 → 0.0`; `cm_t1_tenor: 1 → 3`; `cm_t4_derating: 0.0 → 20.94`; `cm_t4_tenor: 0 → 15` |
| Embedded Benefits | `emb_benefits_switch: 0 → 1` |
| BESS Floor | `bess_floor_switch: 1 → 0`; `bess_floor_rev_share: 10.0 → 9.0` |
| CAPEX | `capex_grid: 30 → 57.858`; `capex_development: 15 → 2.949`; `capex_dd: 5 → 3.775`; `capex_discharge: 0 → 0.983`; `capex_sdlt: 0 → 0.753`; `capex_land_legal: 2 → 3.686`; `capex_other_finance: 0 → 5`; `capex_other_legal: 2 → 0`; `capex_ampyr_tech: 0 → 3.236`; **`capex_bess: 80 → 600`** (the smoking gun); `capex_landowner_fees: 0 → 11.597`; `capex_insurance: 3 → 6.329`; `capex_land_lease_constr: 0 → 2.457`; `capex_misc: 0 → 4.916` |
| Solar OPEX | `opex_grid_conn: 1.5 → 0.003`; `opex_greenkeeping: 0.5 → 1.5`; `opex_community: 0 → 0.5`; `opex_real_estate_tax: 1 → 1.222`; `opex_non_tech_am: 1 → 1.3`; `opex_insurance: 2.02 → 2.021`; `opex_tech_am: 1.5 → 0.3`; `opex_balancing_cfd: 0 → 2.75` |
| BESS OPEX | `bess_opex_om: 7.06 → 7.063`; `bess_opex_rates: 0 → 3.276`; `bess_opex_lease: 0 → 1.489` |
| Land | `fixed_lease_switch: 0 → 1`; `fixed_lease_acres: 200 → 205`; `fixed_lease_price: 800 → 700`; `rev_dep_lease_switch: 0 → 1`; `rev_share_yr11_35: 7.5 → 5.0` |
| WC + Discount | `wc_debtors_days: 45 → 30`; `project_discount_rate: 8.0 → 6.5` |

New keys added (engine adapter reads them; pre-A43 the adapter fell back to engine defaults which are Excel-correct, but explicit init is safer):

- `shl_switch: 1` (SHL on per A21 / Excel `Solar&BESS Inputs!F556`)
- `shl_pct_of_unfunded: 99.0`
- `shl_rate: 15.0` (Excel `Solar&BESS Inputs!F553`)
- `depreciation_method: 'RB'` (Excel `D&T!E165`)
- `depreciation_rate: 5.5556` (= 100 × 2/36, Excel `D&T!E164`)

**Lock-in regression tests** added in `tests/test_wizard_state_path.py`:

1. `test_default_wizard_state_produces_audit_capex` — asserts £101.6m ± £0.5m via the fresh-session path.
2. `test_default_wizard_state_produces_audit_combined` — asserts Combined 8.85% ± 0.1 pp.
3. `test_default_wizard_state_produces_audit_sb` — asserts S+B 9.02% ± 0.1 pp.

Helper: `_run_d13_via_default_wizard_state()` does `deepcopy(DEFAULT_WIZARD_STATE['financial'])` then runs dispatch + PIRR. This is the new "fresh-session" test variant alongside A28's "Step 7-Save" variant — together they cover both entry paths.

**Result (post-A43)**:

| Metric | Pre-A43 fresh-session | Post-A43 fresh-session | Target (D13 audit) |
| --- | --- | --- | --- |
| Combined PIRR | 19.67% | **8.88%** | 8.85% |
| S+B PIRR | 24.01% | **9.05%** | 9.02% |
| Gas PIRR | 12.43% | **13.07%** | 13.07% |
| Total CAPEX | £64.7m | **£101.4m** | ≈ £101.6m |

CAPEX residual £0.2m is rounding noise in the per-capex-line precision.

**Test suite**: 45 passed + 4 xfailed (was 42 + 4 — three new fresh-session tests added).

**Audit script** unchanged: 4 matrix rows still at the same -0.45 to -0.19 pp gap (engine core not touched).

**Files touched**:

- `src/wizard_state.py` — financial section value swaps + 5 new keys + header comment block pointing at the lock-in test.
- `tests/test_wizard_state_path.py` — `_run_d13_via_default_wizard_state` helper + 3 lock-in tests.

**What this means for the open SME questions** (Anchal Q1/Q2): zero impact. The audit matrix engine numbers are unchanged. A43 closes a UI-path correctness bug, not an engine-vs-Excel calibration question.

### A42. sizing_results unification — drop the dead `wizard['results']['simulation_results']` slot (NEW 2026-05-16)

**Status doc P1 cleanup** per A19 (May 12). The phrase "Unify `sizing_results` vs canonical `wizard['results']['simulation_results']`" hid a real fork: the word "canonical" came from A19's comment in Step 7 calling the wizard-state slot "canonical" and "future-state contract" — but the Spec §8 schema explicitly lists `st.session_state.sizing_results` (top-level) as the storage location, and every single call site uses the top-level key. The wizard-state slot has never been written.

**Why drop, not migrate**:

| Signal | Direction |
| --- | --- |
| Spec §8 line 284 explicitly lists `st.session_state.sizing_results` as the schema entry | **Top-level canonical** |
| Step 3 writes top-level (line 422) | **Top-level canonical** |
| Step 3a, Step 4, Step 7 read top-level | **Top-level canonical** |
| Step 1 cache-invalidation pops 6 top-level keys (sizing_results + 5 others) — established convention | **Top-level canonical** |
| `wizard['results']['simulation_results']` is initialized to `None` but never written | Wizard slot is dead |
| `wizard['results']` helper functions (`add_comparison_config`, `set_results_filter` etc.) are imported by zero pages | Whole nested dict largely dead |
| A19 comment in Step 7 calls the wizard slot "canonical" and "future-state contract" | Wizard slot was aspirational |

The aspiration was contradicted by every other piece of code. Unification = remove the aspiration. Migration in the other direction (top-level → wizard nested) would have meant: rewiring Step 1's cache-clear (top-level pop won't reach nested dict), updating Spec §8, and editing 5+ pages — for an architectural tidiness that runs against the established cache pattern.

**Code changes**:

1. [src/wizard_state.py](../src/wizard_state.py) — removed `'simulation_results': None` from `DEFAULT_WIZARD_STATE['results']`. Added a block comment explaining where sizing results actually live (top-level per Spec §8) and flagging the rest of `wizard['results']` as present-but-unused.
2. [pages/Step7_Financial.py](../pages/Step7_Financial.py) — updated the `check_prerequisites()` comment block; removed the stale `Canonical wizard['results']['simulation_results']` line; now correctly documents top-level as canonical.

**Out of scope** (kept as pre-existing dead code per Karpathy guideline 3, "clean up only your own orphans"):

- `wizard['results']` keys still present: `selected_configs`, `sort_column`, `sort_ascending`, `filters`, `detail_view_config`, `ranked_recommendations`, `recommendation_generated`.
- `wizard_state.py` helper functions still present: `add_comparison_config`, `remove_comparison_config`, `clear_comparison_selection`, `set_results_filter`, `toggle_results_filter`.
- None of these are imported by any page (verified via `grep` across `pages/`). They are pre-existing orphans unrelated to A42's task. Flagged in the wizard_state.py block comment for a separate cleanup if desired.

**Verification**:

- Programmatic check: `'simulation_results' not in DEFAULT_WIZARD_STATE['results']` ✓
- Remaining `results` keys after A42: `detail_view_config`, `filters`, `ranked_recommendations`, `recommendation_generated`, `selected_configs`, `sort_ascending`, `sort_column` (all pre-existing dead code, out of A42 scope).
- pytest: 42 pass + 4 xfail unchanged.
- Streamlit boots clean.

**Files touched**: `src/wizard_state.py` (5 lines: removed 1, added 5 of comment), `pages/Step7_Financial.py` (3 lines of comment).

### A41. Step 4 conditional Financial Metrics section — Step 3a Phase 2 deliverable (NEW 2026-05-16)

**Headline**: Step 4 (per-config dispatch drilldown) now surfaces Combined/S+B/Gas PIRR + NPV + CAPEX + Payback for the currently-selected config when the user has previously run Step 3a's financial sweep. Spec D15 + §7 Phase 2 deliverable; A24 explicitly deferred this; cache invalidation hooks already shipped in A27.

**Interpretation note — what "Step 4 augmentation" actually means in current code.** Spec §7 line 263 reads:

> Step 4 Results (now augmented with PIRR/NPV/MOIC columns)

That language presumes a sweep-comparison table in Step 4 — which never got built. The actual Step 4 implementation is a single-config dispatch drilldown (user picks BESS MWh + Container type + DG MW; page runs one 8760-h simulation and shows metrics + monthly summary + hourly chart). Step 3 is where the sweep-results DataFrame is displayed; Step 3a duplicates that table with PIRR/NPV columns added; Step 4 is the per-config deep-dive.

Three interpretations were considered:

1. **Augment Step 3's existing sweep table** with PIRR/NPV when `financial_results` exists. Rejected — Step 3a already does exactly this, the duplication adds clutter without value.
2. **Add a new sweep-comparison table to Step 4** above the drilldown. Rejected — duplicates Step 3a entirely; the user would have to choose which page to visit for the same view.
3. **Augment Step 4's per-config drilldown with PIRR/NPV scalars** for the selected config. **Chosen.** Surgical, non-redundant, aligns with current Step 4 shape, and matches the Spec's intent (financial-aware Step 4) even if not its literal wording about "columns".

**Code changes** ([pages/Step4_Results.py](../pages/Step4_Results.py)):

1. New `find_cached_financial(bess_mwh, dg_mw, container_type)` helper. Mirrors the existing `find_cached_result` against `st.session_state.financial_results` (the DataFrame Step 3a writes). Defensive: returns None when `financial_results` is absent, empty, or missing any of the join columns. Join key: `BESS (MWh)` × `DG (MW)` × `Duration (hr)` (derived from `container_type` via `CONTAINER_SPECS`).
2. New rendering block in the main `if st.session_state.analysis_results is not None:` body, between the "Solar Utilization" metric row and the Monthly Performance Summary table. Three branches:
   - **Match found** → render a `## £ Financial Metrics` section with 6 `st.metric` columns (Combined PIRR / S+B PIRR / Gas PIRR / NPV / CAPEX / Payback). Caption flags the A24 dispatch-module mismatch so the user understands why Step 4's green-% may differ from Step 3a's implicit green-%.
   - **`financial_results` exists but no match** → info banner pointing the user back to Step 3a or asking them to pick a config that's in the sweep.
   - **`financial_results` missing entirely** → nothing rendered. User experience pre-A41 is preserved for anyone who skips Step 3a.

No engine changes. No test changes (the new helper is a pure DataFrame lookup; verified via inline unit test across hit/miss/None/missing scenarios).

**What's NOT shipped in A41**:

- **MOIC.** Spec §7 line 263 mentions PIRR/NPV/**MOIC** columns. Step 3a's `run_pirr_for_config` doesn't compute MOIC today (only Combined/S+B/Gas PIRR, NPV, Total CAPEX, Lifetime Revenue, Payback). Adding it requires a small engine extension (`PirrResults.moic` field) — out of scope for the UI-only Phase 2 augmentation. Logged as a follow-up.
- **Sweep-comparison table in Step 4.** Per the interpretation note above, kept in Step 3a only.
- **In-Step-4 financial assumption overrides.** Spec §7 already allows in-page overrides via Step 3a / Step 7. Adding a third entry point is scope creep.
- **Browser smoke test.** Static checks complete (AST parse + helper unit test + pytest 42 pass + 4 xfail + Streamlit clean boot). Browser walkthrough of Step 1 → Step 3 → Step 3a → Step 4 deferred to user — I can't drive the browser myself.

**Verification**:

```python
# find_cached_financial behaviour smoke-test (run inline)
# - Hit (50 MWh, 25 DG, 2hr container)  → returns matching row, Combined PIRR 8.88%
# - Miss (200 MWh, 25 DG, 4hr container) → None
# - financial_results = None              → None
# - financial_results missing entirely    → None
```

Tests: 42 pass + 4 xfail unchanged. AST parses. Streamlit boots clean.

**Files touched**: `pages/Step4_Results.py` only.
...
**Cleanup follow-up** — the A40 scaffolding-then-wiring sequence accidentally landed a duplicate `cpi_curve_by_calendar_year` field declaration in `PirrInputs` (two `field(default_factory=...)` calls referencing the same `_DEFAULT_CPI_CURVE_BY_CALENDAR_YEAR` factory). Python silently kept the second declaration so behaviour was correct, but the second was sandwiched between the gas opex block and `gas_fuel_price_gbp_mwh` — out of logical position. Removed in `9604e1c` (PIRR A40 follow-up: remove duplicate cpi_curve_by_calendar_year field). The kept declaration sits with the other indexation-related fields alongside `opex_solar_fixed_indexation` / `opex_pv_om_indexation`, with the longer docstring and the `cpi_steady_state_rate` companion field. Verified via `dataclasses.fields(PirrInputs)` — 160 unique names, no shadowing. Audit unchanged.

**Commits**: scaffolding `5fb4648` (PIRR A40 (WIP scaffolding only): time-varying CPI curve); full wiring `38599df` (PIRR A40: time-varying CPI curve in _esc_factor (Excel-faithful)); duplicate-field cleanup `9604e1c` (PIRR A40 follow-up: remove duplicate cpi_curve_by_calendar_year field).
...
**Status doc priority-1 was**: "NOL + dep-from-construction + multi-account split (10-25 bps Combined)". Delivered: **+0.07 pp uniform** (toward the low end of the range, primarily because the NOL pool's tax-shield benefit is partly offset on the gas chain by an NPV penalty from earlier capex deployment).

**Background**: A21 (2026-05-12) added the NOL carry-forward to `_calc_tax` (lines 1233-1240 in pre-A38 source), but the loop was filtered by `is_operations` only, so construction-period depreciation never fed the pool. Result: the pool was *always empty* when ops started, and the engine effectively did `tax = max(0, EBIT × rate)` per year. A35 (2026-05-15) shipped capex per-month phasing infrastructure (`capex_phasing_sb/gas` dicts + Burton-Leonard curves) but defaulted them OFF because phasing-only regressed Combined IRR by ~11 bps in isolation. The 4-phase A38 work activates the dormant NOL pool, makes depreciation start at capex-addition month, and turns the phasing dicts on.

**Code changes**:

1. New module constant `_DEPRECIATION_ACCOUNTS` (3 entries: long_term / short_term / financing) with monthly RB rates and lifetimes per Excel `D&T!r68/r87/r106` and `D&T!r64/r83/r102`.
2. `_calc_capex` refactored to return `(capex, total_capex, additions_by_account)`. SB stream split into `sb_long_term` (solar + BESS + contingency), `sb_financing` (IDC + Fin Fees), `sb_dsra` (cash only — DSRA returns at EOL, not depreciated). Gas all to long_term. Routing matches Excel `Construction!B76:B112` SUMIF tags.
3. `_calc_depreciation` refactored to consume `additions_by_account`. Each account runs its own RB+SLM-crossover chain per addition. Per-addition chain_len = min(account.life_months, n - addition_month).
4. `_calc_tax` mask widened from `is_operations` to `np.ones(n, dtype=bool)` — entire timeline is taxable. Ebitda/interest are zero pre-COD by construction (no revenue, IDC is in capex), so taxable_y = -annual_depr → loss_pool accumulates.
5. `PirrInputs.capex_phasing_sb/gas` defaults flipped from empty `{}` to `dict(_DEFAULT_CAPEX_PHASING_*_BY_MONTH)`.

**Result — phase-by-phase impact**:

| Phase | d13 Combined | d13 S+B | d13 Gas | Note |
| --- | ---: | ---: | ---: | --- |
| Pre-A38 baseline (post-A37) | 8.78% | 8.82% | 14.13% | — |
| + Phase C only (dep-from-construction, single-account) | 8.83% | 8.86% | 14.21% | Uniform +0.05 pp; pool accumulates ~£8M of pre-COD dep |
| + Phase D (A35 phasing on, still single-account) | 8.69% | 8.86% | 12.87% | **Regression** — gas capex shifted ~9 mo earlier; £1.96M NPV penalty vs £241k pool benefit |
| + Phase A (3 accounts) — final | **8.85%** | **9.02%** | 13.07% | Account 3 (financing 5.56%/mo RB) on £2.2M IDC + Fin Fees generates £1.4M extra pre-COD dep → £350k tax shield → flips D net positive |

Cumulative A38 uplift across the 4 audit rows: **+0.07 to +0.09 pp uniform**. Combined gap shifted from -0.30/-0.57 pp to **-0.21/-0.49 pp**.

**Audit matrix post-A38**:

| Case | Combined Eng | Combined Tgt | ΔComb | S+B Eng | Gas Eng |
| --- | ---: | ---: | ---: | ---: | ---: |
| d13 (82/170) | 8.85% | 9.2% | **-0.35** | 9.02% | 13.07% |
| m82_160 (82/160) | 7.31% | 7.8% | **-0.49** | 8.35% | 8.22% |
| m115_170 (115/170) | 9.59% | 9.8% | **-0.21** | 10.43% | 9.86% |
| m115_160 (115/160) | 8.27% | 8.5% | **-0.23** | 9.73% | 5.80% |

**Direction-of-impact notes**:

- S+B uplift (+0.20 pp): NOL pool absorbs ~£20M of pre-COD dep (SB capex £79M × 18 mo of partial-life RB on long_term + £2.2M × 18 mo near-full RB on financing) → big tax shield in early ops years.
- Gas regression on its own chain (-1.06 pp): gas capex deploys avg 9 mo earlier with no offsetting financing-account contribution (gas has no IDC routed to it). NPV penalty dominates the pool benefit on the gas-only XIRR. Combined chain is still net positive because the S+B NOL-pool gain absorbs most of the gas timing penalty when XIRR is on the sum of both FCFFs.
- Gas-only IRR vs Excel: was 14.13% (over by +3.36 pp); now 13.07% (over by +2.30 pp) — closer to Excel 10.77% gas reference, structurally cleaner.

**Tests**: 42 pass + 4 xfail (audit matrix targets unchanged). `test_wizard_state_path` EXPECTED_* re-baselined: Combined 0.0878 → 0.0885; S+B 0.0882 → 0.0902; Gas 0.1413 → 0.1307.

**What's still left** (in priority order, post-A38):

| Item | Est. impact | Notes |
| --- | --- | --- |
| Solar fixed CPI indexation residual | 5-7 bps | A37 identified ~£1,100k aggregate residual across 5 CPI lines (~2.3% over) + Insurance (~3.4% over). Likely anchor-date mismatch — engine starts indexation from COD; Excel uses `Inputs!r293-304 = 2023-03-01`. |
| Construction insurance + terminal land sale + LoC PPA + decomm bond | 10-30 bps total | 4 stubbed/missing capex / revenue / opex items per gap analysis §3.1-3.4. |
| Operational-period capex (BESS replacement, augmentation) | 0 bps D13 | Zero for D13 per A38 diagnostic (`Solar&BESS Operation!r228-r239` all zero). Could matter for other configs. |
