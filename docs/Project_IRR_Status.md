...
**Last updated:** 2026-05-16 (v1 CLOSED at ±0.5 pp tolerance per A45)

**Status: v1 AUDIT CLOSED.** Path 1 decision taken 2026-05-16 by Ankit after the post-A44 follow-up confirmed the residual gap as structural (DSCR sculpting / cash sweep deferred to v2). Engine reproduces Excel within ±0.5 pp across all 4 audit rows (gaps: -0.36 / -0.48 / -0.23 / -0.23 pp; uniform direction; range 0.25 pp; config ranking preserved). Tests: 49 passed, 0 xfailed. Anchal awaiting a brief Teams confirmation.

**v1 framing**: pre-IC screening, what-ifs, sizing comparisons. Excel remains the source-of-truth IRR for IC-pack until v2 closes the residual gap. Production Doublu handoff unblocked.

**v2 scope** (per A45 + Spec §10): DSCR-driven gearing convergence, cash sweep, Equity IRR, gearing-as-sweep-dimension. Estimated 1-2 weeks engineering. Re-evaluated when v1 is in users' hands.
...
| 2026-05-16 (A45) | **v1 CLOSED** at ±0.5 pp tolerance | Path 1 decision (Ankit). Tolerance relaxed 0.001 → 0.005 in `test_project_irr_excel_parity.py`; 4 xfail markers removed; 49 pass + 0 xfail. Residual ~0.3-0.5 pp gap accepted as the v1 structural carve-out (DSCR sculpting + cash sweep + Equity IRR all deferred to v2 per Spec §10). v1 use cases: pre-IC screening / what-ifs / sizing comparisons. Excel remains source-of-truth for IC-pack headline IRR. Production Doublu handoff unblocked. |
